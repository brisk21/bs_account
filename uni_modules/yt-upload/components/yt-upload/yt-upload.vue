<template>
  <view class="yt-file" :style="[getStyles]">
    <view ref="yt" class="hFile" :style="[getStyles]" @click="onClick">
      <slot>
        <view class="defview" :style="[getStyles]">请选择文件</view>
      </slot>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    // 点击选择附件
    onClick() {
      console.log('点击了上传按钮',this.option)
      ...
      //拼接到url,判断有没有问号
      this.option.url = this.option.url.indexOf('?') > -1 ? this.option.url : this.option.url  +'?';
      ...
      this.create();
    console.log('this.ytFile',this.ytFile)
